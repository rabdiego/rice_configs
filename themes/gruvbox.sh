#!/bin/sh

export background='32302f'
export backerground='000000'
export foreground='fbf1c7'
export main='d79921'
export highlight='689d6a'
export highlight2='458588'

export black='282828'
export red='cc241d'
export green='98971a'
export yellow='d79921'
export blue='458588'
export magenta='b16286'
export cyan='689d6a'
export white='ebdbb2'

export bblack='3c3836'
export bred='fb4934'
export bgreen='b8bb26'
export byellow='fabd2f'
export bblue='83a598'
export bmagenta='d3869b'
export bcyan='8ec07c'
export bwhite='ffffff'

export wallpaper='/home/diego/Documents/projects/rice_configs/wallpapers/gruvbox.png'